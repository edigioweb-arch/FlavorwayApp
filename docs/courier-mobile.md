...
## LOT 9 — finalisation du parcours livreur

- L’assignation compare `courier_city_id` au `city_id` de l’adresse figée dans la commande, pour les deux types de livreurs. Une ville absente/invalide ou un livreur sans ville rend le candidat inéligible. Le nom libre d’une ville et la ville du restaurant ne remplacent pas cette donnée. L’Admin reçoit un message explicite si aucun candidat ne convient. Aucune commande historique n’est modifiée.
- Disponible reste : compte actif et aucune autre livraison `confirmed`, `preparing`, `ready`, `picked_up` ou `on_the_way`. Parmi les candidats de la bonne ville, priorité au livreur du restaurant, puis fallback FlavorWay. Une commande livrée avec Cash restant ne bloque pas la disponibilité, mais reste visible et actionnable.
- Dashboard et liste active : actualisation toutes les 30 secondes ; historique : 60 secondes ; détail : 10 secondes. Les requêtes périodiques cessent quand l’application est en arrière-plan ou qu’une autre route recouvre l’écran. Pas de requêtes périodiques concurrentes sur le même écran. Tirer pour actualiser reste disponible.
- « Encaissements à confirmer » affiche le nombre et le montant serveur de toutes les livraisons Cash non encaissées, avec un aperçu de cinq commandes et un accès à la liste complète paginée. Ces commandes restent aussi dans la liste des commandes et dans l’historique, avec un badge orange. L’encaissement utilise toujours le service financier existant.
- Notifications : même collection Laravel, pagination de 30, lu/non lu, marquage individuel, compteur global fourni par Laravel et ouverture par référence serveur. La cloche et l’inbox se rafraîchissent toutes les 30 secondes au premier plan. FCM reste le transport push existant ; aucun test automatisé ne certifie à lui seul une réception sur appareil.
- Le profil est relu depuis Laravel à l’ouverture et sur actualisation manuelle. Seul le téléphone est éditable. Un rafraîchissement de profil ne réinitialise pas la déduplication des notifications.
- Le détail affiche les consignes, repère et note déjà présents dans la réponse. Le téléphone client n’est ajouté que dans le détail autorisé du livreur assigné, pendant une livraison active ; il n’est pas retourné dans l’historique ou après livraison. Aucun email client ni autre donnée de profil n’est ajouté.
